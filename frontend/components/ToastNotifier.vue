<template>
    <div class="flex flex-col fixed translate-x-1/2 right-1/2 bottom-6 w-80 gap-3 z-50">
        <Transition v-for="it in toastList" :key="it"
        ...
        leave-to-class="transform opacity-0">
            <div class="flex flex-1 items-center rounded shadow py-2 px-3 gap-3 bg-white text-xs sm:text-sm" :class="{'cursor-pointer': it.copyable}"
             v-if="it.visible" @click="it.copyable ? copy() : _">
...
<script setup>
    ...
    const show = async (message, type = 'success', options) => {
        const timeout = options?.timeout ?? 3000;
        const closable = options?.closable ?? true;
        const copyable = options?.copyable ?? true;
        // init
        const obj = { message: message, type: type, closable: closable, copyable: copyable, visible: false, date: Date.now() };
        // push
        toastList.value.push(obj);
        await nextTick();
        toastList.value.find((it) => it.date == obj.date).visible = true;
        await new Promise(r => setTimeout(r, timeout));
        // remove
        const o = toastList.value.find((it) => it.date == obj.date)
        if (o) o.visible = false;
        await nextTick();
        const i = toastList.value.findIndex((it) => it.date == obj.date);
