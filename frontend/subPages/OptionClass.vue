<template>
    <div class="flex flex-col gap-3">
        ...
        <div class="flex flex-col gap-2 lg:flex-row">
            <OptionClassCreate class="grow-[1] lg:basis-1"
            @on-created="classPaginator?.reload()"
            v-if="authStore?.hasAnyRole([UserRole.Editor])"></OptionClassCreate>
            ...
            <OrderTable id="id" :headers="headers" :rows="classList">
                ...
                <template #操作="{ id }">
                    <div class="flex gap-2 text-base">
                        <Icon icon="ic:round-mode-edit" class="cursor-pointer" @click="optionClassEditPopUp?.show(id)"
                        v-if="authStore?.hasAnyRole([UserRole.Editor])"></Icon>
                        <Icon icon="ic:round-delete" class="cursor-pointer text-red-600" @click="optionClassDeletePopUp?.show(id)"
                        v-if="authStore?.hasAnyRole([UserRole.Editor])"></Icon>
                    </div>
                </template>
            </OrderTable>
