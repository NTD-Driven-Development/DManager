<template>
    <div class="flex flex-col gap-3">
        ...
        <RecordPointDeletePopUp ref="recordPointDeletePopUp" @on-edited="pointLogPaginator?.reload()"></RecordPointDeletePopUp>
    </div>
</template>
...
<script setup lang="ts">
    ...
    const pointLogPaginator = new PointLogPaginator();
    const { data: pointLogList } = pointLogPaginator;

    pointLogPaginator?.bind('project_id', toRef(props, 'projectId'));
</script>